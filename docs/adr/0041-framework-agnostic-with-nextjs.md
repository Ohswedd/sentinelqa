...
Next.js support." (ADR-0010) shipped an HTTP-first crawler
that targets any web app; (ADR-0022) added the Playwright
backend for SPAs. (ADR-0031) ships demo apps in Next.js,
...
- **Diff-aware test selection **: ships deterministic Next.js (App Router + Pages Router) and Vite heuristics for translating a git diff into impacted routes. Other frameworks fall back to the "broad-impact tripwire" path (run everything).
- **Examples**: five reference apps span Next.js / FastAPI / Django / Flask / Vite. The audit produces the same artifact tree for every framework.
...
- **Next.js-only release.** Rejected — narrows the addressable audience to one corner of the web ecosystem and risks framework-specific shortcuts leaking into the core.
...
- Related ADRs: ADR-0010 (Discovery release), ADR-0022 (CI integration), ADR-0031 (Example apps)
