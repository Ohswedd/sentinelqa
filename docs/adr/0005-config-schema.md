...
SentinelQA needs a single, strictly-validated configuration surface (`sentinel.config.yaml`) that the CLI, SDK, modules, and CI all read identically. PRD §17 sketches the YAML layout; CLAUDE.md §12 spells out the discipline (safe defaults, explicit errors, no silent dangerous fallback, env interpolation, secret redaction, unknown-key rejection, CI-safe behaviour without prompts).
...
- **Wildcards forbidden.** `target.allowed_hosts` rejects any host containing `*` or `?`. The safety boundary in CLAUDE.md §6 hinges on the allow-list being explicit; a wildcard would make the policy lie.
...
- **Allow inline secrets and rely on git hooks to catch them.** Rejected. Defense in depth: gitleaks (pre-commit) catches commits; the loader catches misuse of the file. Both. CLAUDE.md §33 demands secrets never enter logs or reports — which is impossible to guarantee if the config can carry them.
...
- PRD section(s): PRD §17 (Configuration Specification), PRD §18 (Data Model — Project / Target).
- CLAUDE.md rule(s): CLAUDE.md §12 (Config rules), CLAUDE.md §33 (Logging & secrets), CLAUDE.md §6 (Safety boundary), CLAUDE.md §34 (Required ADR triggers — "Config schema").
