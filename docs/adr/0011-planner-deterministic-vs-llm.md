...
The Phase 06 planner is **deterministic by default; an LLM adapter can augment — never replace — the deterministic output, behind a feature flag**.
...
6. Each accepted proposal becomes a `Flow` with `source="llm"`, `extractor="llm.v<PROMPT_VERSION>"`, and at least the `"llm"` tag. The runner (Phase 08+) and scoring (Phase 14) can therefore distinguish deterministic vs. LLM-sourced flows.
