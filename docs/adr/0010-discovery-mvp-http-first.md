...
Phase 05 builds the Discovery module (the documentation): the component that produces a
...
- the documentation — DiscoveryGraph + RiskMap are the contract Phase 06+ depend on, regardless of how they were produced.
...
also the slice most often produced by LLM coding agents (the documentation / Phase 19
...
2. Phase 17 already wires the modes (fast/standard/full/nightly/release) that decide when a browser is worth spinning up.
3. The Python orchestrator pattern Phase 17 owns is the cleanest place to land a Playwright-driven discovery backend as a swap-in for the HTTP one.
...
- The `Crawler` is declared via a `CrawlBackend` Protocol; the HTTP backend is the only one shipped in Phase 05.
...
A Playwright-driven backend that handles CSR SPAs lands in Phase 17 as (added in
...
- **Positive:** Phase 05 ships a complete, testable, deterministic discovery pipeline. Every Phase 06+ module gets a real `DiscoveryGraph` to consume. The HTTP backend is fast enough to run on every PR (no Chromium needed), which keeps the default CI lane cheap. The `CrawlBackend` Protocol is the contract Phase 17 plugs into — no rework of the consuming modules when the second backend arrives.
- **Negative / trade-off:** Pure CSR SPAs (Vite + React with no SSR) produce a near-empty `<body>` over HTTP, so the HTTP backend reports a small or empty `DiscoveryGraph` for them. Users hitting that case get a clear log warning and a `RiskMap` finding (`risk_model.rule = "spa_empty_body"`) telling them to switch to `discovery.engine: playwright` once Phase 17 ships it.
- **Follow-up obligations:** 1. Phase 17 must deliver task 07 (Playwright discovery backend) before the LLM-code audit phase (Phase 19) can fairly run against CSR SPAs. **Resolved by Phase 17 task 07 (see ):** `engine/discovery/backends/playwright_backend.py` ships the `PlaywrightCrawlBackend`; the new `sentinel-ts discover` subcommand drives Chromium; `discovery.page` + `discovery.endpoint` event kinds are added to `ts-events.schema.json` and parsed by `engine.orchestrator.ts_bridge`; the CSR-SPA gate test runs under the new GitHub Actions lane `discovery-playwright (gated)` behind `SENTINELQA_HAS_CHROMIUM=1`. 2. the documentation is updated in this PR to reflect the HTTP-first MVP and the Phase 17 backend roadmap. 3. The `discovery.engine` config key is reserved in the loader so the Phase 17 backend can be enabled without a schema bump.

## Alternatives considered

- **Build the Playwright backend now alongside the HTTP one.** Rejected because it would require a new TS-side discovery driver, a bidirectional RPC over the JSONL bridge (Phase 04 designed the bridge as one-way), and significant test coverage for both backends — too much scope to absorb alongside the other seven Phase 05 tasks without diluting either.
- **Ship a single Playwright-only backend and drop the HTTP path entirely.** Rejected because it makes the default CI lane depend on Chromium for every PR; raises the floor for contributors; and Phase 04 already gates Chromium behind `SENTINELQA_HAS_CHROMIUM=1`, signalling that browser- dependent paths should be opt-in until Phase 17 wires the CI mode.
- **Defer the whole `sentinel discover` CLI to Phase 17.** Rejected because Phase 06 (Planner) and Phase 07 (Generator) consume `DiscoveryGraph`; if Phase 05 ships only the engine code without the CLI surface, the next two phases have no way to drive discovery end-to-end and would have to invent ad-hoc harnesses.
...
- Related ADRs: [ADR-0009](./0009-python-ts-protocol.md) (Python ↔ TS JSONL protocol — the bridge the Phase 17 backend will use).
