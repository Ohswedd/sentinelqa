...
- **Negative / trade-off:** Coverage gate is configured-but-soft today. Mitigated by owning the flip, with the trigger date already pegged to "first redaction commit." If lands without enabling the gate, the phase gate review fails.
- **Follow-up obligations:** enables `make coverage` inside `make ci` and pins the floor at 80. Every subsequent phase that adds a new package must also add it to the coverage `source = [...]` list.
...
- **Enforce coverage floor at by excluding the empty source dirs.** Rejected: gaming the floor (running coverage on test files only) earns a 100% number that means nothing. A flag-day flip at the first real code commit is honest and easy to audit.
