...
SentinelQA spans two runtimes (Python orchestration, TypeScript Playwright runtime), multiple deliverables (CLI, SDK, MCP server, modules, integrations, examples, docs), and a 30-phase execution plan that adds code phase by phase. the documentation prescribes the exact monorepo layout; our engineering rules

## Decision

Adopt the monorepo layout in the documentation verbatim:
...
- the documentation Repository structure, §11.3 Language strategy.
- our engineering rules§43 Implementation Order (item 1).
