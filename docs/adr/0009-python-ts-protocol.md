...
builds that boundary. We needed to decide:
...
- A future (Runner) consumes one async-iterable surface (`stream_events`) and doesn't need to know about Playwright at all.
...
- (Runner) wires `sentinel-ts run` into the run lifecycle and starts feeding stdout into `stream_events`.
- (Quality Scoring) and (HTML & JSON Reports) treat the JSONL stream as the canonical source for test outcomes — no parallel "real" reporter.
- (MCP & Agent Interface) re-exports the event types as agent messages; any rename there must propagate to this schema.
