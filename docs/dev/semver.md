...
The release calendar is driven by the engineering milestones in our product spec and the recommended build order in our product spec Each tag is a real artifact produced by `make build-all` and inspected by `make inspect-all` (Task 28.04). No tag is published without a sign-off line in the pre-tag review process (Task 28.06).
