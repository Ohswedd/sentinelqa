...
CLAUDE.md §33 / §41: the webhook URL is a secret — it is never logged
...
```
python -m integrations.slack.poster \
    --payload .sentinel/runs/<run-id>/slack.json \
    --webhook-env SLACK_WEBHOOK_URL \
    --dedup-cache .sentinel/runs/<run-id>/slack-dedup.json
```
...
serialized dict and pushes it::

    from integrations.slack import post_payload
    post_payload(
        payload=block_kit_dict,
        webhook_url=os.environ["SLACK_WEBHOOK_URL"],
        dedup_path=Path(".sentinel/runs/<id>/slack-dedup.json"),
    )
